...
- **✅ FULL duplicate-`en` audit (slice-5 REFLECT — the whole last-wins defect class, sized).**
  ...
  **📌 CURRENT SKIP SET (authoritative — supersedes the scattered slice-4/5 notes above; the journal/plan skip-list is ADVISORY & stale-prone, so the truth is THIS list ∧ author-time `byEn` resolution).** Skip when authoring a slice:
  ...
  - **SKIP pending /workshop fix — `program` プロクラム (missing-dakuten typo for プログラム, caught slice-7).** Do NOT author a sentence embedding the typo kana. Un-skip once words.json is fixed (プロクラム→プログラム + regen Nanami audio + add プロクラム to kana-smoke #8 ban-list). Queued in journal DEFERRED as a /workshop task.
  - **SKIP pending /workshop fix — `soft` やらかい (missing-MORA typo for やわらかい 柔らかい "soft", caught slice-8).** Drops the わ mora — reads wrong though all chars are valid. Do NOT author a sentence embedding the typo kana. Un-skip once words.json is fixed (やらかい→やわらかい + regen Nanami audio + add やらかい to kana-smoke #8 ban-list). Queued in journal DEFERRED as a /workshop task.
  - **⚠ VALID-CHARS-WRONG-READING TYPO CLASS is RECURRING, not a one-off (broadened slice-8 REFLECT — was "missing-dakuten", too narrow).** THREE confirmed instances now: `kitchen` だいところ→だいどころ (missing-dakuten, slice-4) · `program` プロクラム→プログラム (missing-dakuten, slice-7) · `soft` やらかい→やわらかい (missing-MORA, slice-8). The unifying class is **kana where every character is individually valid but the word READS wrong** — a dropped ゛ (dakuten) OR a dropped mora (わ). The junk-detector passes them because no char is illegal. **Author-time guard for every slice: eyeball each candidate's kana against its expected reading** — for katakana loanwords compare vs the English↔katakana transliteration; for native words sanity-check the mora count/dakuten against the kanji you'd expect. The guard is unchanged — only its NAME was too narrow (a missing-mora slips a "missing-dakuten"-labelled check). Systematic fix (all rows vs a dictionary/ban-list, catching both sub-classes) is queued for /workshop; until then this per-slice eyeball is the guard.
  ...
  - **📍 slice-9 frontier: next 50 MISSING alphabetical by `en` start AFTER `summer`** (≈ sun/sunday/sweat/t-shirt/table/tall/teacher/team/teardrop…). ~172 remain after slice-8 (364→414). sentences.json is INSERTION-ORDER — enumerate missing keys against images.json slugs, do NOT assume contiguity. (slice-8 shipped science→summer, sw v18.)

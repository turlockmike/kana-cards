...
- **✅ FULL duplicate-`en` audit (slice-5 REFLECT — the whole last-wins defect class, sized).**
  ...
  **📌 CURRENT SKIP SET (authoritative — supersedes the scattered slice-4/5 notes above; the journal/plan skip-list is ADVISORY & stale-prone, so the truth is THIS list ∧ author-time `byEn` resolution).** Skip when authoring a slice:
  ...
  - **📍 slice-11 frontier: next ~50 MISSING alphabetical by `en` start AFTER `to-touch`** (≈ to-turn/to-walk/to-wash/to-wear.../to-win/to-work/to-write… then the non-`to-` tail: town/toy/train/tree/uncle/vegetable/village… — enumerate live, do NOT trust this sketch). **~72 remain after slice-10 (514/586).** sentences.json is INSERTION-ORDER — enumerate missing keys against `media/img/images.json` slugs (the app builds cards from images.json; the `en`↔`slug` join is hyphen-vs-space, so join on images.json's own `en` field, NOT by string-munging the slug). **Watch for kanji-in-kana-field verb rows → author with the pure-kana reading (see to-think below); and the `to-catch`-class sokuon/dakuten/mora typos.**

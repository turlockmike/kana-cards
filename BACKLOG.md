...
- **✅ FULL duplicate-`en` audit (slice-5 REFLECT — the whole last-wins defect class, sized).**
  ...
  **📌 CURRENT SKIP SET (authoritative — supersedes the scattered slice-4/5 notes above; the journal/plan skip-list is ADVISORY & stale-prone, so the truth is THIS list ∧ author-time `byEn` resolution).** Skip when authoring a slice:
  ...
  - **⚠ MISSING-DAKUTEN TYPO CLASS is RECURRING, not a one-off (slice-7 REFLECT).** Two confirmed instances now: `kitchen` だいところ→だいどころ (slice-4/workshop) and `program` プロクラム→プログラム (slice-7). Loanword/native kana that drop a ゛ (dakuten) render a plausible-looking but wrong reading the junk-detector misses (all chars are valid kana). **Author-time guard for every slice: eyeball each candidate's kana for a plausibly-missing dakuten** (esp. katakana loanwords — compare against the expected English↔katakana transliteration). Systematic fix (all katakana rows vs a dictionary/ban-list) is queued for /workshop; until then this per-slice eyeball is the guard.
  ...
  - **📍 slice-8 frontier: next 50 MISSING alphabetical by `en` start at `science` かがく** (~222 remain after slice-7's 314→364). sentences.json is INSERTION-ORDER — enumerate missing keys, do NOT assume contiguity.

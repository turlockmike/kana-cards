...
- **✅ FULL duplicate-`en` audit (slice-5 REFLECT — the whole last-wins defect class, sized).**
  ...
  **📌 CURRENT SKIP SET (authoritative — supersedes the scattered slice-4/5 notes above; the journal/plan skip-list is ADVISORY & stale-prone, so the truth is THIS list ∧ author-time `byEn` resolution).** Skip when authoring a slice:
  ...
  - **⚠ VALID-CHARS-WRONG-READING TYPO CLASS is RECURRING, not a one-off (broadened slice-8 REFLECT — was "missing-dakuten", too narrow).** THREE confirmed instances, ALL NOW FIXED: `kitchen` だいところ→だいどころ (missing-dakuten, slice-4) · `program` プロクラム→プログラム (missing-dakuten, slice-7, fixed 7/24) · `soft` やらかい→やわらかい (missing-MORA, slice-8, fixed 7/24). The unifying class is **kana where every character is individually valid but the word READS wrong** — a dropped ゛ (dakuten) OR a dropped mora (わ). The junk-detector passes them because no char is illegal. **NEW 4th instance found slice-9 (7/24), STILL OPEN: `to-catch` キャチ → should be キャッチ (missing SOKUON っ — a 3rd sub-class alongside missing-dakuten & missing-mora).** SKIPPED in slice-9 (not authored). Fix pending: words.json キャチ→キャッチ + regen word audio + add キャチ to kana-smoke #8 BAN (+ PIN[to-catch]... but note `to-catch` is the SLUG; byEn key is `to catch` → confirm the `en` field) — same recipe as program/soft. **Class status: 1 known-open (`to-catch`); missing-dakuten + missing-mora + missing-sokuon are all the SAME class (valid chars, wrong reading). The systematic all-rows-vs-dictionary scan is still queued for /workshop to surface any UNKNOWN instances — it should test char-count/sokuon/dakuten against expected transliteration.** **Author-time guard for every slice: eyeball each candidate's kana against its expected reading** — for katakana loanwords compare vs the English↔katakana transliteration; for native words sanity-check the mora count/dakuten against the kanji you'd expect. The guard is unchanged — only its NAME was too narrow (a missing-mora slips a "missing-dakuten"-labelled check). Systematic fix (all rows vs a dictionary/ban-list, catching both sub-classes) is queued for /workshop; until then this per-slice eyeball is the guard.
  ...
  - **📍 slice-10 frontier: next 50 MISSING alphabetical by `en` start AFTER `to-find`** (≈ to-fly/to-follow/to-get-up/to-get-wet/to-go/to-grow/to-hang-up/to-hear/to-jump…). ~122 remain after slice-9 (414→464). sentences.json is INSERTION-ORDER — enumerate missing keys against images.json slugs, do NOT assume contiguity. **Watch for kanji-in-kana-field verb rows (買う/電話/あの人たち etc.) → skip or author with the pure-kana reading; and the `to-catch`-class sokuon/dakuten/mora typos.** (slice-9 shipped sun→to-find, sw v20, +50; SKIPPED they/to-buy/to-call/to-catch.)
